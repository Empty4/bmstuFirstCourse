...
int main() {
    ...
    outputFile << "Исходный контейнер:" << endl;
    for (const Deposit& deposit : deposits) {
        outputFile << deposit << endl;
    }

    // Сортировка контейнера
    sort(deposits.begin(), deposits.end(), compareDepositsByAmount);

    // Вывод отсортированного контейнера
    outputFile << "Отсортированный контейнер:" << endl;
    for (const Deposit& deposit : deposits) {
        outputFile << deposit << endl;
    }

    // Копирование в новый контейнер
    vector<Deposit> copiedDeposits(deposits.size());
    copy(deposits.begin(), deposits.end(), copiedDeposits.begin());

    // Вывод скопированного контейнера
    outputFile << "Скопированный контейнер:" << endl;
    for (const Deposit& deposit : copiedDeposits) {
        outputFile << deposit << endl;
    }
}